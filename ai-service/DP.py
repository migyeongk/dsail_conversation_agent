...
1. 도입 및 라포형성: 챗봇이 사용자와 처음 만나 신뢰를 형성하고 대화의 분위기를 만드는 전략
    ...
    1-2. ask_current_state: 본격적인 대화 전, 사용자의 현재 기분이나 컨디션에 대해 물으며 자연스럽게 대화를 시작하는 전략
...
5. 대화 관리: 대화 흐름에서 벗어나는 상황을 처리하고 대화의 목적을 유지하는 전략
    ...
    5-4. explain_limitations: 챗봇의 능력을 벗어나는 요청 시, 할 수 없는 일임을 명확히 알리는 전략
    5-5. handle_no_response: 사용자가 질문에 대해 무응답했을 때, 대답하지 않아도 괜찮음을 언급하고 자연스럽게 다음 질문으로 넘어가는 전략 
...
7. 대화 흐름의 유연성 및 다양성 보장:
    ...
    - question이 들어가는 정책을 한 턴에서 두 번 선택하지마세요. (예: question, ask_new_question을 동시 선택하지 마세요)
8. 정책 선택 불가 시:
